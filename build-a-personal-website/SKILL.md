...
Fill in `name`, `tagline`, `photo`, `socials`. For the look (colors, fonts, borders, effects), if the person wants to design it visually, **open the `style-kit.html` that ships with the template** (see `reference/style-kit.md`) — **do NOT build a new picker or artifact**. They design their look on a live preview, hit **Copy my look**, and tell you the settings; you translate them into `site.config.js → theme` (mapping table in `reference/style-kit.md`). Every option applies to the real site. Text auto-sits readably on whatever accent they choose. If unsure, keep the default beige/lime/Geist.
